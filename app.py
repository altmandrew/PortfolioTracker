...
<style>
    ...
    div[data-testid="stMetricValue"] { color: #e8eaed !important; }
    ...
    with st.form("Login"):
        ...
        if st.form_submit_button("Log in"):
            try:
                ...
                st.error("Error reading accounts")
                st.exception(e)
    return False
...
with st.sidebar.expander("➕ Add Asset", expanded=False):
    ...
    with st.form("add_asset_form"):
        if a_type == "Option":
            st.caption("Premium is **per share**. 1 contract = 100 shares.")
...
with tab_home:
    ...
    with col_type:
        port_chart_type = st.selectbox("Type", ["Candlestick", "Line"], index=0,
                                       label_visibility="collapsed", key="port_chart_type")
    ...
    if not hist_df.empty:
        ...
        if len(filtered) >= 2:
            ...
            filtered["Open"] = filtered["Close"].shift(1).fillna(filtered["Close"].iloc[0])
            ...
            if port_chart_type == "Line":
                ...
                fig.add_trace(go.Candlestick(
                    x=filtered["Date"], open=filtered["Open"], high=filtered["High"],
                    low=filtered["Low"], close=filtered["Close"],
                    increasing_line_color="#00d1b2", decreasing_line_color="#ff6b6b",
                    increasing_fillcolor="#00d1b2", decreasing_fillcolor="#ff6b6b"))
    ...
    if holdings.empty:
        ...
        display_df = holdings[["Symbol","Type","Quantity","Average Cost","Price",
                               "Market Value","Weight (%)","P&L ($)"]].copy()
        ...
        with col_sym:
            default_sym = st.session_state.get("QQQ", "QQQ")
    ...
    def get_cnn_fear_greed_30d():
        try:
            import fear_greed
            history = fear_greed.get_history(last="365")   # last 365 days
    ...
    if holdings.empty:
        ...
        cash_pct = cash_val / total * 100 if total else 0
        bond_pct = bond_val / total * 100 if total else 0
        option_pct = option_val / total * 100 if total else 0
        equity_pct = equity_val / total * 100 if total else 0

        has_leveraged = any((holdings["Type"] == "Option") &
                            (holdings["Symbol"].str.contains("TQQQ|SQQQ|UPRO|SPXU|TNA|TZA", case=False, na=False)))

        if has_leveraged and option_pct > 10:
            style, best, desc, risk = ("Aggressive / High-Beta Growth", "Strong Bull Market",
                                       "Built for maximum upside with leveraged long calls.",
                                       "High volatility and large drawdown risk.")
        elif equity_pct > 70 and option_pct < 10 and bond_pct < 20:
            style, best, desc, risk = ("Growth-Oriented Equity", "Bull Market",
                                       "Primarily equity-focused with limited defensive holdings.",
                                       "Vulnerable to broad market pullbacks.")
        elif bond_pct > 30 or (bond_pct + cash_pct) > 40:
            style, best, desc, risk = ("Balanced / Defensive", "Sideways to Mildly Bearish",
                                       "Meaningful allocation to bonds/cash provides ballast.",
                                       "Will lag in strong bull markets.")
        else:
            style, best, desc, risk = ("Moderately Aggressive", "Bull Market with moderate volatility",
                                       "Mix of growth exposure and some defensive elements.",
                                       "Still carries meaningful equity risk.")

        col1, col2 = st.columns(2)
        with col1:
            st.metric("Portfolio Style", style)
            st.metric("Best Suited Market", best)
        with col2:
    ...
    with col1:
        annual_contribution = st.slider("Annual Contribution ($)", 0, 100000, 22000, 1000)
    with col2:
        expected_return = st.slider("Expected Annual Return (%)", 0.0, 25.0, 12.0, 0.5)
    with col3:
        years = st.slider("Time Horizon (Years)", 1, 50, 25)
    ...
    fig_proj = go.Figure(go.Scatter(
        x=years_list, y=values, mode="lines",
        line=dict(color="#00d1b2", width=2.5),
        fill="tozeroy", fillcolor="rgba(0,209,178,0.08)"))
    fig_proj.update_layout(template="plotly_dark", paper_bgcolor="rgba(0,0,0,0)",
...
with tab_news:
    ...
    for name, cfg in CHANNELS.items():
        ...
        if not videos:
            st.info(f"Could not load videos for {name}.")
            continue
